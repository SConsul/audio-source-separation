...
class MixedSquaredError(nn.Module):
    ...
    def forward(self, pred_bass, pred_vocals, pred_drums, pred_others, gt_bass, gt_vocals, gt_drums, gt_others):
        ...
        L_sq = torch.sum((pred_bass-gt_bass).pow(2)) + torch.sum((pred_vocals -
                                                                  gt_vocals).pow(2)) + torch.sum((pred_drums-gt_drums).pow(2))
        ...
        L_diff = torch.sum((pred_bass-pred_vocals).pow(2)) + torch.sum((pred_bass -
...
def train():
    ...
    for epoch in range(num_epochs):
        ...
        for i, (inp, gt_bass, gt_vocals, gt_drums, gt_others) in enumerate(train_loader):
            ...
            mask_bass, mask_vocals, mask_drums, mask_others = TimeFreqMasking(
                o_bass, o_vocals, o_drums, o_others, cuda)
            pred_drums = inp*mask_drums
            pred_vocals = inp*mask_vocals
            pred_bass = inp*mask_bass
            pred_others = inp*mask_others

            loss = criterion(pred_bass, pred_vocals, pred_drums, pred_others,
                             gt_bass, gt_vocals, gt_drums, gt_others)
        ...
        for i, (val_inp, gt_bass, gt_vocals, gt_drums, gt_others) in enumerate(val_loader):
            ...
            vloss = criterion(pred_bass, pred_vocals, pred_drums, pred_others,
                              gt_bass, gt_vocals, gt_drums, gt_others)
            writer.add_scalar('Validation loss', vloss, epoch)
            val_loss.update(vloss.item(), inp.size(0))

        print("Epoch {}, Training Loss: {}, Validation Loss: {}".format(
            epoch+1, train_loss.avg(), val_loss.avg()))
